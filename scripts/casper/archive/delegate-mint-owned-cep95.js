...
async function delegateMint() {
  console.log("🎯 Delegate Mint via Authority Mint → OwnedCep95\n");
  ...
  console.log("OwnedCep95:", OWNED_CEP95_HASH);
  ...
  const metadata = CLValueBuilder.list([
    CLValueBuilder.tuple2([
      ...
      CLValueBuilder.string("Delegate Minted NFT"),
    ]),
    CLValueBuilder.tuple2([
      CLValueBuilder.string("symbol"),
      CLValueBuilder.string("DELNFT"),
    ]),
    CLValueBuilder.tuple2([
  ...
  // CEP-78 metadata format (JSON string) for Authority Mint
  const metadataJson = JSON.stringify({
    name: "Delegate Minted NFT",
    token_uri: "https://raw.githubusercontent.com/dellwatson/moga-mogate/refs/heads/master/metadata/v2-test/nfts/casper/tixia/1o1/201/metadata.json"
  });
  ...
  try {
    ...
    if (
      ...
      if (executionResult.Success) {
        ...
        console.log(
          "   Authority Mint → OwnedCep95 cross-contract call succeeded!"
        );
        console.log("   NFT minted via delegated authority!");
        console.log("");
        console.log("📊 Result saved to: delegate-mint-proof.json");

        // Save proof
        const fs = require("fs");
        fs.writeFileSync(
          "delegate-mint-proof.json",
          JSON.stringify(
            {
      ...
      console.log("⚠️  Deploy still pending, check later");
    }
  } catch (error) {
