...
LATEST_DEPLOY="9cb0a6e123a503a9766d66c53c21f73c0c774a1aaf1a92655a0fba64fbdf7095"
AUTHORITY_HASH="hash-011b472d6ba72303df22357de62f347b6f4dd0aac4d2804fa3e1604e00a4065a"

echo "🔍 Checking LATEST PUBLIC CEP-95 (500 CSPR gas)"
echo "================================================"
echo ""
echo "Deploy: $LATEST_DEPLOY"
echo "Explorer: https://testnet.cspr.live/deploy/$LATEST_DEPLOY"
echo ""

STATUS=$(curl -s -X POST $NODE_URL/rpc \
  -H "Content-Type: application/json" \
  -d "{\"jsonrpc\":\"2.0\",\"method\":\"info_get_deploy\",\"params\":{\"deploy_hash\":\"$LATEST_DEPLOY\"},\"id\":1}" \
...
if [[ "$STATUS" == *"SUCCESS"* ]]; then
  ...
  echo "✅ Authority Mint: $AUTHORITY_HASH"
  echo ""
  ...
  cat > deployment-casper/READY-TO-MINT.json <<EOF
{
  ...
  "publicCep95": {
    "deployHash": "$LATEST_DEPLOY",
    "contractHash": "$PUBLIC_CEP95_HASH",
    "status": "SUCCESS",
    "payment": "500 CSPR",
    "explorerLink": "https://testnet.cspr.live/deploy/$LATEST_DEPLOY"
  },
  "authorityMint": {
...
EOF
  ...
  echo "Check explorer for details: https://testnet.cspr.live/deploy/$LATEST_DEPLOY"
else
  echo "⏳ Still deploying... check again in 1-2 minutes"
  echo "   ./scripts/casper/check-latest-cep95.sh"
fi
