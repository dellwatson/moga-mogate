...
const PUBLIC_CEP95_HASH = process.argv[2]; // Pass as argument
const PAYMENT_AMOUNT = "3000000000";

if (!PUBLIC_CEP95_HASH || !PUBLIC_CEP95_HASH.startsWith("hash-")) {
  console.error(
    "❌ Usage: node whitelist-authority-mint.js <public-cep95-hash>"
  );
  console.error("   Example: node whitelist-authority-mint.js hash-abc123...");
  process.exit(1);
}

async function whitelistCollection() {
  console.log("🔐 Whitelisting Public CEP-95 in Authority Mint\n");
  ...
  console.log("Collection to whitelist:", PUBLIC_CEP95_HASH);
  console.log("");
  ...
  try {
    ...
    if (
      ...
      if (executionResult.Success) {
        ...
        console.log(
          "   Public CEP-95 is now allowed to be minted by Authority Mint!"
        );
        console.log("");
        console.log("🎯 Next step: Run delegate mint script");
