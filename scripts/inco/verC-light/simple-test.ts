/**
 * Simple Test for verC-light
 *
 * Raw transaction test to verify deployed LIGHT program works (Auto-Assigned Numbers)
 *
 * Usage:
 *   bun run scripts/inco/verC-light/simple-test.ts
 */
...
// verC-light instruction data serializer (auto-assigned numbers)
function serializeHostRaffleData(
  ...
  // Max number (auto-assigned range)
  buffer.writeUInt32LE(maxNumber, offset);
  offset += 4;
...
async function main() {
  console.log(
    "🎯 Simple test for verC-light (Auto-Assigned Numbers + LIGHT + FHE)",
  );
  ...
  // Test parameters (auto-assigned numbers)
  const raffleId = `verC-test-${Date.now()}`;
  const maxNumber = 1000000; // Range 1-1M for auto-assignment
  const metadataUri = "https://example.com/verC-test";
  ...
  try {
    ...
    console.log(`🔢 Auto-assigned range: 1-${maxNumber.toLocaleString()}`);
    ...
    // Skip config initialization for verC-light (not required)
    console.log(`⚙️ Skipping config (verC-light doesn't need it)`);
    ...
    const instruction = new TransactionInstruction({
      keys: [
        { pubkey: walletKeypair.publicKey, isSigner: true, isWritable: true }, // authority (first, must sign)
