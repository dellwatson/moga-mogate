...
async function main() {
  ...
  console.log(
    `Verifying MogateERC721Vault at ${address} on ${network} (chainId: ${chainId})...`,
  );

  // Read contract source
  const __dirname = path.dirname(new URL(import.meta.url).pathname);
  const repoRoot = path.join(__dirname, "..", "..");
  const contractPath = path.join(repoRoot, "contracts", "Vault.erc721.sol");
  ...
  const erc721ReceiverPath = path.join(
    repoRoot,
    "node_modules",
    "@openzeppelin",
    "contracts",
  ...
  const erc721ReceiverSource = fs.readFileSync(erc721ReceiverPath, "utf8");
  ...
  const sourceCode = JSON.stringify({
    language: "Solidity",
    sources: {
      "contracts/Vault.erc721.sol": { content: contractSource },
  ...
  const params = new URLSearchParams({
    ...
    contractname: "contracts/Vault.erc721.sol:MogateERC721Vault",
