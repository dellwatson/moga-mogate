...
__export(exports_evm, {
  unsafeJoinRaffleWithReport: () => unsafeJoinRaffleWithReport,
  unsafeHostRaffleWithReport: () => unsafeHostRaffleWithReport,
  signJoinRafflePermit: () => signJoinRafflePermit,
  signHostRafflePermit: () => signHostRafflePermit,
  signHostAndJoinRafflePermit: () => signHostAndJoinRafflePermit,
  joinRaffleWithPermit: () => joinRaffleWithPermit,
  hostRaffleWithPermit: () => hostRaffleWithPermit,
  hostAndJoinRaffleWithPermit: () => hostAndJoinRaffleWithPermit,
  hashJoinRafflePermit: () => hashJoinRafflePermit,
  hashHostRafflePermit: () => hashHostRafflePermit,
  hashHostAndJoinRafflePermit: () => hashHostAndJoinRafflePermit,
  getRaffleDomainFromClient: () => getRaffleDomainFromClient,
  getRaffleContract: () => getRaffleContract,
  createRaffleClientFromSigner: () => createRaffleClientFromSigner,
  createRaffleClientFromProvider: () => createRaffleClientFromProvider,
  createRaffleClient: () => createRaffleClient,
  buildRaffleDomain: () => buildRaffleDomain,
  RAFFLE_EIP712_DOMAIN: () => RAFFLE_EIP712_DOMAIN,
  RAFFLE_ABI: () => RAFFLE_ABI,
  PrizeTokenType: () => PrizeTokenType,
  JOIN_RAFFLE_TYPES: () => JOIN_RAFFLE_TYPES,
  HOST_RAFFLE_TYPES: () => HOST_RAFFLE_TYPES,
  HOST_AND_JOIN_RAFFLE_TYPES: () => HOST_AND_JOIN_RAFFLE_TYPES
});
  ...
  static from(sig) {
    ...
    if (typeof sig === "string") {
      ...
      if (bytes3.length === 65) {
        ...
        const v2 = Signature.getNormalizedV(bytes3[64]);
...
var PrizeTokenType;
((PrizeTokenType2) => {
  PrizeTokenType2[PrizeTokenType2["NONE"] = 0] = "NONE";
  PrizeTokenType2[PrizeTokenType2["ERC721"] = 1] = "ERC721";
  PrizeTokenType2[PrizeTokenType2["ERC1155"] = 2] = "ERC1155";
  PrizeTokenType2[PrizeTokenType2["ERC404"] = 3] = "ERC404";
})(PrizeTokenType ||= {});
var RAFFLE_EIP712_DOMAIN = {
  name: "MogateRaffle",
  version: "1"
};
var HOST_RAFFLE_TYPES = {
  HostRaffle: [
...
var HOST_AND_JOIN_RAFFLE_TYPES = {
  HostAndJoinRaffle: [
    { name: "raffleId", type: "string" },
    { name: "totalSlots", type: "uint256" },
    { name: "maxSlotsPerAddress", type: "uint256" },
...
function getRaffleContract(raffleAddress, signerOrProvider) {
  return new exports_ethers.Contract(raffleAddress, RAFFLE_ABI, signerOrProvider);
}
function createRaffleClient(config) {
  const provider = new exports_ethers.JsonRpcProvider(config.rpcUrl);
  const signer = config.privateKey ? new exports_ethers.Wallet(config.privateKey, provider) : undefined;
  const raffle = getRaffleContract(config.raffleAddress, signer ?? provider);
  return { provider, signer, raffle };
}
function createRaffleClientFromSigner(config) {
...
function createRaffleClientFromProvider(config) {
  const raffle = getRaffleContract(config.raffleAddress, config.provider);
  return { provider: config.provider, raffle };
}
async function unsafeHostRaffleWithReport(client, params) {
  if (!client.signer) {
    throw new Error("unsafeHostRaffle requires a signer");
  }
  const { raffle, signer, provider } = client;
  const network = await provider.getNetwork();
  const tx = await raffle.unsafeHostRaffle(params.raffleId, params.totalSlots, params.maxSlotsPerAddress, params.metadataUri, params.collection, params.premintContract, params.premint, params.prizeType, params.prizeAmount, params.autoDraw, params.autoClaim, params.expiresAt);
  const receipt = await tx.wait();
  const id2 = exports_ethers.id(params.raffleId);
...
async function unsafeJoinRaffleWithReport(client, params, valueOverride) {
  if (!client.signer) {
    throw new Error("unsafeJoinRaffle requires a signer");
  }
  const { raffle, signer, provider } = client;
  const network = await provider.getNetwork();
  const tx = await raffle.unsafeJoinRaffle(params.raffleId, params.slotIds, params.amount, params.token, { value: valueOverride ?? params.amount });
  const receipt = await tx.wait();
  const load = await raffle.getRaffleLoadDetail(params.raffleId);
  const result = await raffle.getRaffleResult(params.raffleId);
  const loadReport = {
...
function buildRaffleDomain(chainId, verifyingContract) {
  return {
...
async function getRaffleDomainFromClient(client) {
...
function hashHostRafflePermit(domain, message) {
  return exports_ethers.TypedDataEncoder.hash(domain, HOST_RAFFLE_TYPES, message);
}
function hashJoinRafflePermit(domain, message) {
  return exports_ethers.TypedDataEncoder.hash(domain, JOIN_RAFFLE_TYPES, message);
}
function hashHostAndJoinRafflePermit(domain, message) {
  return exports_ethers.TypedDataEncoder.hash(domain, HOST_AND_JOIN_RAFFLE_TYPES, message);
}
async function signHostRafflePermit(signer, domain, message) {
  return signer.signTypedData(domain, HOST_RAFFLE_TYPES, message);
}
async function signJoinRafflePermit(signer, domain, message) {
  return signer.signTypedData(domain, JOIN_RAFFLE_TYPES, message);
}
async function signHostAndJoinRafflePermit(signer, domain, message) {
  return signer.signTypedData(domain, HOST_AND_JOIN_RAFFLE_TYPES, message);
}
async function hostRaffleWithPermit(client, params, signature) {
  if (!client.signer) {
    throw new Error("hostRaffle requires a signer");
  }
  const { raffle, signer, provider } = client;
  const network = await provider.getNetwork();
  const tx = await raffle.hostRaffle(params.raffleId, params.totalSlots, params.maxSlotsPerAddress, params.metadataUri, params.collection, params.premintContract, params.premint, params.prizeType, params.prizeAmount, params.autoDraw, params.autoClaim, params.expiresAt, signature);
  const receipt = await tx.wait();
  const id2 = exports_ethers.id(params.raffleId);
  const load = await raffle.getRaffleLoadDetail(params.raffleId);
  const result = await raffle.getRaffleResult(params.raffleId);
...
async function joinRaffleWithPermit(client, params, signature, valueOverride) {
  if (!client.signer) {
    throw new Error("joinRaffle requires a signer");
  }
  const { raffle, signer, provider } = client;
  const network = await provider.getNetwork();
  const tx = await raffle.joinRaffle(params.raffleId, params.slotIds, params.amount, params.token, signature, { value: valueOverride ?? params.amount });
  const receipt = await tx.wait();
  const load = await raffle.getRaffleLoadDetail(params.raffleId);
  const result = await raffle.getRaffleResult(params.raffleId);
  const loadReport = {
...
async function hostAndJoinRaffleWithPermit(client, params, signature, valueOverride) {
  if (!client.signer) {
    throw new Error("hostAndJoinRaffle requires a signer");
  }
  const { raffle, signer, provider } = client;
  const network = await provider.getNetwork();
  const tx = await raffle.hostAndJoinRaffle(params.raffleId, params.totalSlots, params.maxSlotsPerAddress, params.metadataUri, params.collection, params.premintContract, params.premint, params.prizeType, params.prizeAmount, params.autoDraw, params.autoClaim, params.expiresAt, params.slotIds, params.amount, params.token, params.bonusFreeSlots, signature, { value: valueOverride ?? params.amount });
  const receipt = await tx.wait();
  const id2 = exports_ethers.id(params.raffleId);
  const load = await raffle.getRaffleLoadDetail(params.raffleId);
  const result = await raffle.getRaffleResult(params.raffleId);
...
// src/tee/index.ts
var exports_tee = {};
__export(exports_tee, {
  getTeeRaffleContract: () => getTeeRaffleContract,
  createTeeRaffleClientFromSigner: () => createTeeRaffleClientFromSigner,
  createTeeRaffleClientFromProvider: () => createTeeRaffleClientFromProvider,
  createTeeRaffleClient: () => createTeeRaffleClient,
  buildSlotCommitment: () => buildSlotCommitment,
  RAFFLE_TEE_ABI: () => RAFFLE_TEE_ABI,
  PrizeTokenType: () => PrizeTokenType2,
...
var RAFFLE_TEE_ABI = [
  ...
  "function joinSlotsOnlyWithSlots(string raffleId,uint256[] slotIds,bytes32[] commitments) payable",
  ...
  "function isSlotTaken(string raffleId,uint256 slotId) view returns (bool)",
  "function getSlotStatusBatch(string raffleId,uint256[] slotIds) view returns (bool[])",
  "function getCommitments(string raffleId) view returns (bytes32[])",
  "function getCommitmentsCount(string raffleId) view returns (uint256)"
];
// src/tee/client.ts
var PrivacyMode;
((PrivacyMode2) => {
  PrivacyMode2[PrivacyMode2["SLOTS_ONLY"] = 0] = "SLOTS_ONLY";
  PrivacyMode2[PrivacyMode2["FULL"] = 1] = "FULL";
})(PrivacyMode ||= {});
var PrizeTokenType2;
((PrizeTokenType3) => {
  PrizeTokenType3[PrizeTokenType3["NONE"] = 0] = "NONE";
  PrizeTokenType3[PrizeTokenType3["ERC721"] = 1] = "ERC721";
  PrizeTokenType3[PrizeTokenType3["ERC1155"] = 2] = "ERC1155";
  PrizeTokenType3[PrizeTokenType3["ERC404"] = 3] = "ERC404";
})(PrizeTokenType2 ||= {});
function getTeeRaffleContract(raffleAddress, signerOrProvider) {
  return new exports_ethers.Contract(raffleAddress, RAFFLE_TEE_ABI, signerOrProvider);
}
function createTeeRaffleClient(config) {
  const provider = new exports_ethers.JsonRpcProvider(config.rpcUrl);
  const signer = config.privateKey ? new exports_ethers.Wallet(config.privateKey, provider) : undefined;
  const raffle = getTeeRaffleContract(config.raffleAddress, signer ?? provider);
  return { provider, signer, raffle };
}
function createTeeRaffleClientFromSigner(config) {
  const provider = config.signer.provider;
  if (!provider) {
    throw new Error("Signer must be connected to a provider");
  }
...
function createTeeRaffleClientFromProvider(config) {
  const raffle = getTeeRaffleContract(config.raffleAddress, config.provider);
  return { provider: config.provider, raffle };
}
function buildSlotCommitment(params) {
  const raffleIdHash = exports_ethers.id(params.raffleId);
  return exports_ethers.keccak256(exports_ethers.solidityPacked(["bytes32", "uint256", "bytes32", "address"], [raffleIdHash, params.slotId, params.salt, params.buyer]));
}
export {
  exports_tee as tee,
  exports_evm as evm
